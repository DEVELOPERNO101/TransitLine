...
-- Ordered stops along a bus's route, between its current GPS position and the school.
CREATE TABLE IF NOT EXISTS stops (
  id        SERIAL PRIMARY KEY,
  bus_name  TEXT NOT NULL REFERENCES buses(bus_name) ON DELETE CASCADE,
...
-- Students, each linked to the stop where their bus picks them up.
...
-- Tracks the last date "present" was reset for everyone, so a new day starts fresh.
CREATE TABLE IF NOT EXISTS daily_state (
  id               INT PRIMARY KEY DEFAULT 1,
  last_reset_date  DATE NOT NULL DEFAULT '2000-01-01',
  CONSTRAINT daily_state_single_row CHECK (id = 1)
);
